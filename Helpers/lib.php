...
// === MASTER FUNTION FOR EVERY WHERE ==== //
...
function qrcode($data)
{
    ...
    if (isset($data)) {
        ...
        // user data
        $filename = $PNG_TEMP_DIR . 'codeflies' . md5($data . '|' . $errorCorrectionLevel . '|' . $matrixPointSize) . '.png';
...
function check_list($name, $array_list, $selected = null, $height = '160px')
{
    ...
    ?>
    <span class='btn btn-sm btn-info float-right' onclick="selectcheck('<?php echo $name; ?>')"><i class='fa fa-check'></i></span>
    <hr>
    <?php
